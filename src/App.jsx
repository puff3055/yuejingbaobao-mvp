      ...
      <div className="mobile-prototype onboarding">
        ...
        {step === 5 && (
          <OnboardingStep eyebrow="记忆由妳决定" title="哪些东西被允许留下？" copy="数据默认保存在这台设备，不会自动上传云端。需要联网对话或加入社区时，妳再单独决定。">
            ...
            <PrivacyRow icon={ChatCircleDots} title="允许宝宝在需要时联网回应" copy="开启后，只有妳主动发送的当次消息会交给在线智能服务生成回复；默认关闭" checked={profile.agentCloudConsent} onChange={(v) => setProfile((p) => ({ ...p, agentCloudConsent: v }))} />
  ...
  return (
    <div className="modal-layer agent-layer" style={{ "--agent-viewport-height": `${viewportHeight}px` }}>
      ...
      <div ref={chatThreadRef} className="chat-thread" aria-live="polite">
        ...
        {analysis && step === "safety" && <div className="safety-card" role="alert"><WarningCircle weight="fill" /><div><p className="eyebrow">产品安全提醒 · 独立于宝宝回复</p><h3>{analysis.redFlag.title}</h3><p>{analysis.redFlag.action}</p><div className="boundary-note">普通对话无法判断原因，也不能替代及时的专业评估。</div><div className="safety-actions">{emergencySafety && <a className="primary-button" href="tel:120">联系紧急帮助（中国大陆 120）</a>}<button className={emergencySafety ? "secondary-button" : "primary-button"} onClick={onClose}>{emergencySafety ? "我已看见" : "我会尽快联系医疗专业人员"}</button></div></div></div>}
